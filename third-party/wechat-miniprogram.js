...
class WechatMiniProgram {
  ...
  async code2session (code) {
    try {
      let res = await axios.post(`https://api.weixin.qq.com/sns/jscode2session?appid=${this.appid}&secret=${this.secret}&js_code=${code}&grant_type=authorization_code`)
  ...
  async sendTemplateMessage ({ access_token,
    ...
    data,
    emphasis_keyword = '' }) {
    let res = await axios.post(`https://api.weixin.qq.com/cgi-bin/message/wxopen/template/send?access_token=${access_token}`, {
      touser,
      template_id,
      page,
      form_id,
   ...
   * @returns {String=[unknown,male,female]}
   */
